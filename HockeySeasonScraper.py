...
class HockeySeasonScraper(SportsDataScraper):
    _url_base = 'http://www.hockey-reference.com/leagues/NHL_' + SportsDataScraper._year_token + '.html'
    _config = SportConfig.NHL()

    @staticmethod
    def __get_url(year):
        return re.sub(HockeySeasonScraper._year_token, str(year), HockeySeasonScraper._url_base)
    ...
    def __assemble_dataset(all_stats):
        ...
        for year in all_stats:
            # remove header categorizing the stat, and last row with calculated 'League Average' stats
            year_stats = all_stats[year][1:-1]

            '''Let's add a header for the Team Name field they left unlabeled.
            Maybe they omit it so you don't join on it. For example, the Winnipeg Jets (2011-)
            are not the same team as the Winnipeg Jets (1979-1995); the former were previously 
            known as the Atlanta Thrashers and the latter are now known as the Arizona Coyotes.'''
            ...
            if len(year_stats) > 1:  # add $year, to the front of every other (non-header) line in the file
                year_stats = ['{0},{1}'.format(str(year), item) for item in year_stats[1:]]

            if found_header_row and len(header_row):
                year_stats.insert(0, header_row)  # put the header row back up top

            year_stats_string = ''.join(year_stats)
            with io.StringIO(year_stats_string) as csv_file:
                reader = list(csv.reader(csv_file))
                if reader:  # I'm still cursing the 2004-2005 lockout a decade later!
                    year_dataframe = pd.DataFrame(data=reader[1:], columns=reader[0], index=None)
